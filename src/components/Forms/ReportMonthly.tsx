...
import {  getCurriculums, getFollowingTypes, getGrades, getSubjects } from '@/services/get_all_select'
...
const ReportMonthly: React.FC = () => {
  ...
  useEffect(() => {
    ...
    getFollowingTypes().then((data: Options[]) =>
      setFollowingTypes(data.map(f => ({ value: f.id.toString(), label: f.x_name })))
    ).catch(console.error);
  ...
  try {
    ...
    data.type1Ids?.forEach((id) => formData.append("type1Ids[]", id.toString()));

    if (data.subject2Id) formData.append("subject2Id", data.subject2Id);
    data.type2Ids?.forEach((id) => formData.append("type2Ids[]", id.toString()));

    if (data.subject3Id) formData.append("subject3Id", data.subject3Id);
    data.type3Ids?.forEach((id) => formData.append("type3Ids[]", id.toString()));

    if (data.subject4Id) formData.append("subject4Id", data.subject4Id);
    data.type4Ids?.forEach((id) => formData.append("type4Ids[]", id.toString()));

    if (data.additionalSubjects) formData.append("additionalSubjects", data.additionalSubjects);
    if (data.achievedGoals) formData.append("achievedGoals", data.achievedGoals);
    if (data.challengesAndSolutions) formData.append("challengesAndSolutions", data.challengesAndSolutions);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full mt-6 p-4">
      ...
      <div className="flex flex-col gap-4">
        <CustomField required label="Description " type="text" {...register("description", { required: true })} />
        {errors.description && <p className="text-red-500 text-sm">Required</p>}

        <CustomField required label="Reservation Id " type="text" {...register("reservationId", { required: true })} />
        {errors.reservationId && <p className="text-red-500 text-sm">Required</p>}
        ...
        <CustomField required label="Hours Spent" type="number" {...register("hoursSpent")} />
        <CustomField required label="Date From" type="date" placeholder="MM-DD-YYYY" {...register("dateFrom")} />
        <CustomField required label="Date To" type="date" placeholder="MM-DD-YYYY" {...register("dateTo")} />
...
onChange={(option) => field.onChange((option as SingleValue<Option>)?.value || "")}
...
onChange={(option) => field.onChange((option as SingleValue<Option>)?.value || "")}
        ...
        />
        {errors.curriculumId && <p className="text-red-500 text-sm">Required</p>}
              {/* Subject 1 */}
        <Controller
          name="subject1Id"
          control={control}
...
<Controller
  ...
  render={({ field }) => (
    <SelectField
      ...
      isMulti
      value={followingTypes.filter(ft => field.value?.includes(Number(ft.value)))}
      onChange={(options) => {
        const selected = (options as MultiValue<Option>).map(o => Number(o.value));
...
onChange={(option) => field.onChange((option as SingleValue<Option>)?.value || "")}
      ...
      onChange={(options) => {
        ...
        />
        <CustomField label="Additional Subjects" type="textarea" {...register("additionalSubjects")} />

        <CustomField required label="Achieved Goals " type="textarea" {...register("achievedGoals", { required: true })} />
        {errors.achievedGoals && <p className="text-red-500 text-sm">Required</p>}

        <CustomField required label="Challenges & Solutions" type="textarea" {...register("challengesAndSolutions")} />

        <CustomField required label="Study Plan " type="textarea" {...register("studyPlan", { required: true })} />
